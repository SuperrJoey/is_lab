    ...
    for rec in records:
        ...
        if docinfo:
            ...
            try:
                ...
                import hashlib
                H = int.from_bytes(
                    hashlib.md5(report_bytes + rec["timestamp"].encode()).digest(),
                    "big"
                ) % (p - 1)
        ...
        if ts:
            ...
            ok_ts = (delta >= -300)  # not in future >5 min

        print(
            f"- report by {did} file={os.path.basename(rec['saved_path'])} "
            f"sig_ok={ok_sig} ts_ok={ok_ts} ts={rec['timestamp']} md5={rec['md5_hex']}"
        )
